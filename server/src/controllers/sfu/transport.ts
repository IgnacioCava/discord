...
import { createRoom, getRoom, removeRoom } from "@services/sfu/room";
import {
  createConsumer,
  createProducer,
  createWebRtcTransport,
} from "@services/sfu/transport";
...
export const connectTransportController = async (socket: Socket) => {
  socket.on(
    "connect-transport",
    async (args: IConnectTransportEventArgs, callback) => {
      ...
      try {
        ...
        console.log(
          `connect-transport: transport connected for user ${peerId}`
        );
...
export const produceController = async (socket: Socket) => {
  ...
  socket.on(
    "close-producer",
    async (args: { producerId: string; roomId: string }) => {
      const { producerId, roomId } = args;

      try {
        const room = getRoom(roomId);

        for (const [remotePeerId, peer] of room.peers) {
          if (remotePeerId === socket.id) {
            for (const producer of peer.producers.values()) {
              if (producer.id === producerId) {
                producer.close();
                socket
                  .to(roomId)
                  .emit("closed-remote-producer", {
                    producerId,
                    kind: producer.kind,
                    peerId: remotePeerId
                  });
